import * as React from 'react'

const HeartSVGComponent = ({className}) => (
  <svg className={className} width="34" height="30" viewBox="0 0 34 30" fill="none" xmlns="http://www.w3.org/2000/svg">
    <path
      d="M13.8914 3.1945C9.15773 -1.21423 -2.58917 7.44006 3.91925 15.1502L14.9953 27.4105C15.7719 28.2702 17.1142 28.2922 17.9186 27.4585L29.7943 15.1502C31.3817 13.3669 33.474 7.91213 29.7943 4.45041C26.0987 0.973774 20.5872 1.77545 16.7774 5.7092L8.0179 13.8944"
      stroke="#fff"
      strokeWidth="3"
      strokeLinejoin="round"
    />
    <circle cx="24.0263" cy="10.1053" r="3.97368" stroke="#fff" strokeWidth="2" />
  </svg>
)

export default HeartSVGComponent
